    ...
    def build(self, nbOfServersPerRegion=5, nbOfClientsPerRegion=2, nbOfRegions=2, **opts):
        ...
        for regionId in range(0, nbOfRegions):
            ...
            for serverId in range(0, nbOfServersPerRegion):
                ...
                servers.append(self.addHost("vessel%d" % (
                    globalId+1), ip=("10.1.0.%d/24" % (globalId+1))))
                # We add link towards the reginal switch
                self.addLink(switches[regionId], servers[globalId], bw=regionalLinkBandwidth,
                             loss=regionalLinkLosses, delay="%dms" % regionalDelay)
            ...
            for clientId in range(0, nbOfClientsPerRegion):
                ...
                # we create the server
                clients.append(self.addHost("client%d" % (
                    globalId+1), ip=("10.1.0.%d/24" % (100+globalId))))
                # We add link towards the reginal switch
                self.addLink(switches[regionId], clients[globalId], bw=regionalLinkBandwidth,
                             loss=regionalLinkLosses, delay="%dms" % regionalDelay)
            # We must also connect the regional switch to the central switch
            self.addLink(centralSwitch, switches[regionId], bw=globalLinkBandwidth,
                         loss=globalLinkLosses, delay="%dms" % globalDelay)
# ------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------
    ...
    def startServer(self, server):
        ...
        makeTerm(node=server, cmd="python {} --id {} --vessels {}".format(self.pathToServer,
                                                                          server.IP().replace('10.1.0.', ''), self.nbOfServersPerRegion*self.nbOfRegions))
# ------------------------------------------------------------------------------------------------------
    # run(self)
    # Run the lab 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Run the distributed system. Launches a Mininet environment composed of multiple servers running your implementation of the lab, as well as a few clients. At startup, launches a firefox instance to test your blackboard.')
    parser.add_argument('--servers', nargs='?', dest='nb_srv', default=10, type=int,
                        help='The number of servers that should be running. If the number is even, the servers will be run in different regions. If the number is odd, all servers will be connected to the same switch.')
    parser.add_argument('--vessels', nargs='?', dest='pth_srv',
                        default='server/server.py', help='The path to your server implementation.')
    args = parser.parse_args()
    nbOfRegions = 2 if args.nb_srv % 2 == 0 else 1
    nbOfServersPerRegion = int(args.nb_srv/nbOfRegions)
    nbOfClientsPerRegion = 2

    lab = Lab(nbOfServersPerRegion, nbOfClientsPerRegion,
              nbOfRegions, args.pth_srv)
    lab.run()
# ------------------------------------------------------------------------------------------------------
